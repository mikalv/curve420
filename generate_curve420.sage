...
# 2. Check consistency between A and d for a=-1
assert A == 2*(a + d)/(a - d)
...
# 6. Map to Edwards coordinates with scaling
c_squared = -4/(1+d)
print(f"c_squared = {c_squared}")
print(f"is c_squared a square? {c_squared.is_square()}")
print(f"is -d a square? {(-d).is_square()}")
c = c_squared.sqrt()
...
params = {
    ...
    "edwards": {
        ...
        "scaling_factor": str(int(c))
    }
}

with open('curve420_generated.json', 'w') as f:
    json.dump(params, f, indent=4)

print("\nSuccessfully generated canonical parameters and saved to curve420_generated.json")
